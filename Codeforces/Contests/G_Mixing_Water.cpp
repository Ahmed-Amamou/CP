...
// get the inputs;
// min = 10^9
// start adding and dividing by the number;
// if the current difference is smaller than the min then min become that value
// if at one point the difference starts is not smaller than the current min;
// then return the past value
//nvm :( maths works
...
{   ll a , b ,t;
    ...
    {
        int k = (t - a) / (a + b - 2 * t);
        double cas1 = fabs(1.0 * ((a + b - 2 * t) * k + a -t) / (2 * k + 1));
        double cas2 = fabs(1.0 * ((a + b - 2 * t) * (k + 1) + a - t) / (2 * k + 3));
        if (cas1 > cas2)
            k++;
        cout << k * 2 + 1 << endl;
    }
